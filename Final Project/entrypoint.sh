...
ENABLE_AIRFLOW_ADD_USER_GROUP="${ENABLE_AIRFLOW_ADD_USER_GROUP:-false}"
...
airflow users create -e FreeUni@airflow.edu -f Hello -l FreeUni -p airflow -r Admin -u airflow 
...
if check_set "${ENABLE_AIRFLOW_INITDB}"; then
  ...
  gosu "${AIRFLOW_USER}" airflow db init || gosu "${AIRFLOW_USER}" airflow db init
