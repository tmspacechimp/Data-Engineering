...
class ExternalTaskSensor(BaseSensorOperator):
    ...
    @apply_defaults
    def __init__(self,
                 # tuple of dags and tasks
                 external_tasks=None,
                 allowed_states=None,
                 execution_delta=None,
                 execution_date_fn=None,
                 check_existence=False,
                 time_delta=datetime.timedelta(seconds=120),
                 *args,
                 **kwargs):
        super(ExternalTaskSensor, self).__init__(*args, **kwargs)
        self.allowed_states = allowed_states or [State.SUCCESS]
        for external_dag_id, external_task_id in external_tasks:
            if external_task_id:
    ...
    def poke(self, context, session=None):
        if self.execution_delta:
            ...
            dttm = self._handle_execution_date_fn(context["execution_date"])
        else:
        ...
        self.time = self.time.replace(tzinfo=dttm_filter[0].tzinfo)

        for dag_id, task_id in self.external_tasks:
            if task_id:
                count = session.query(TI).filter(
                    TI.dag_id == dag_id,
                    TI.task_id == task_id,
                    TI.state.in_(self.allowed_states),
                    TI.execution_date.between(self.time, self.time + self.time_delta)
                ).count()
            else:
                count = session.query(DR).filter(
                    DR.dag_id == dag_id,
                    DR.state.in_(self.allowed_states),
                    DR.execution_date.between(self.time, self.time + self.time_delta)
                ).count()
